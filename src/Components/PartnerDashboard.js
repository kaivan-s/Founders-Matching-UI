...
const PartnerDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async (abortSignal = null) => {
    console.log('PartnerDashboard fetchDashboardData - Called with user?.id:', user?.id, 'abortSignal:', !!abortSignal);
    if (!user?.id) {
    ...
    try {
      ...
      if (profileResponse.ok) {
        ...
        console.log('PartnerDashboard - Profile response:', profileData);
  ...
  // Determine current tab based on route (no need for state since PartnerNavigation handles navigation)
  ...
  useEffect(() => {
    ...
    if (hasFetchedOnceRef.current && !loading && !profile && !error && !hasRedirectedRef.current && location.pathname !== '/partner/onboarding') {
      ...
      redirectTimeoutRef.current = setTimeout(() => {
        ...
        if (!profile && !hasRedirectedRef.current && location.pathname !== '/partner/onboarding') {
          console.log('PartnerDashboard - No profile found after fetch, redirecting to onboarding');
  ...
  // Debug logging
  console.log('PartnerDashboard render - loading:', loading, 'profile:', !!profile, 'profile data:', profile, 'error:', !!error, 'hasFetchedOnce:', hasFetchedOnceRef.current);
  ...
  if (loading && !hasFetchedOnceRef.current) {
    console.log('PartnerDashboard - Showing loading spinner (initial load)');
    return (
  ...
  if (loading && hasFetchedOnceRef.current && profile) {
    ...
    console.log('PartnerDashboard - Loading additional data, showing existing profile');
  }

  if (error) {
    console.log('PartnerDashboard - Showing error:', error);
    return (
      <Box sx={{ p: 3 }}>
  ...
  if (!profile && !loading && hasFetchedOnceRef.current) {
    console.log('PartnerDashboard - No profile after fetch, redirect will handle');
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
        <CircularProgress />
      </Box>
  ...
  console.log('PartnerDashboard - Profile exists, rendering dashboard content, status:', profile?.status);
